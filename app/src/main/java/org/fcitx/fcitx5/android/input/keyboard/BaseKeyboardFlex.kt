...
import org.fcitx.fcitx5.android.input.keyboard.KeyboardLayout
...
abstract class BaseKeyboardFlex(
    context: Context,
    protected val theme: Theme,
    private val keyLayout: List<KeyDef>
    ...
    init {
        ...
        keyLayout.forEach { def ->
            ...
            val lp = KeyboardLayout.LayoutParams(
                def.layoutParam.row,
                def.layoutParam.col,
                def.layoutParam.rowSpan,
                def.layoutParam.colSpan
            )
            addView(keyView, lp)
        }
    }
    ...
    private fun findTargetChild(x: Float, y: Float): View? {
        // 触摸点是相对于 KeyboardLayout 的坐标
        val xInt = x.toInt()
        val yInt = y.toInt()
        for (i in 0 until childCount) {
            val child = getChildAt(i)
            if (child is KeyView) {
                // child.left/top/right/bottom 是相对于 KeyboardLayout 的
                if (xInt >= child.left && xInt < child.right && yInt >= child.top && yInt < child.bottom) {
                    return child
                }
            }
        }
